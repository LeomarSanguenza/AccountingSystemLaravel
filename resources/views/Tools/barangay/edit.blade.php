...
<div class="max-w-2xl mx-auto bg-white p-6 rounded-2xl shadow">
    <div class="mb-4 text-sm text-gray-500 flex items-center space-x-1">
        ...
        <span class="text-gray-700 font-semibold">Barangay</span>
    </div>
    <h1 class="text-2xl font-bold mb-4">Edit Barangay</h1>

    <form action="{{ route('tools.barangays.update', $barangay->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <a href="{{ route('tools.barangays.index') }}" class="ml-2 text-gray-600">Cancel</a>
    </form>
</div>
@endsection
